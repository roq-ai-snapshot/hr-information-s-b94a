      ...
      <Box rounded="md">
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box borderRadius="10px" border="1px" borderColor={'base.300'} mt={6} p={'18px'}>
              <UserPageTable
                createHandler={(values: RoqTypes.hr_manager) => roqClient.hr_manager.create({ data: values })}
                deleteHandler={(id: string) => roqClient.hr_manager.delete({ where: { id } })}
                fetcher={() => roqClient.hr_manager.findManyWithCount({})}
                entity="hr_manager"
                title="Hr Managers"
                filters={{ company_id: id }}
                hidePagination={true}
